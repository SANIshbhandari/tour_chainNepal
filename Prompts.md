...
1. **Two backends doing the same job.** `backend/` re(Express + Mongoose + MongoDB, port 3001) duplicates what Supabase + Next.js API routes should own. One route crashes on invocation (`actions.js:64`, undefined `connection`), three have zero auth, one falls back to a Windows-only keypair path.
